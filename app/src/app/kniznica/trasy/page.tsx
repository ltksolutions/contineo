...
export default async function TracksPage({
  ...
  return (
    <div className="obal" style={{ padding: "28px 20px 80px", maxWidth: 760, ...tenantStyle(branding) }}>
      <Notice message={message ?? error} error={Boolean(error)} back="/kniznica/trasy" />

      <p style={{ margin: "0 0 16px" }}>
        <Link className="tichy" href="/kniznica" style={{ fontSize: 14 }}>
        ...
        {tracks.map(tr => (
          <li key={tr.key} className="karta" style={{ padding: "16px 18px" }}>
            <div style={{ display: "flex", gap: 12, alignItems: "baseline", flexWrap: "wrap" }}>
              <Link href={`/kniznica/trasy/${encodeURIComponent(tr.key)}`} style={{ fontSize: 16.5, fontWeight: 600, flex: "1 1 240px" }}>
                {tr.title}
              </Link>
              <span
                className="stitok"
