...
export async function createTrackAction(fd: FormData) {
  ...
  try {
    ...
    revalidatePath("/kniznica/trasy")
    // Rovno do detailu: po založení nasleduje skladanie krokov a hľadať
    // novú trasu v zozname je zbytočný krok.
    back(`/kniznica/trasy/${encodeURIComponent(key.toLowerCase())}`, { msg: say(self.language).created })
  } catch (e) {
    if (isRedirect(e)) throw e
    back("/kniznica/trasy", { error: message(e, self.language), key, title: text(fd, "title") })
  }
}
...
export async function renameTrackAction(fd: FormData) {
  ...
  const key = text(fd, "key")
  const to = `/kniznica/trasy/${encodeURIComponent(key)}`
  try {
    await renameTrack(
