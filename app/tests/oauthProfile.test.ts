...
describe("Microsoft (Entra ID)", () => {
  it("konto z povoleného tenanta prejde", () => {
    const r = verifyOAuthProfile("microsoft", ms(), { allowedTenantIds: [TID_SFZ] })
  ...
  it("profil bez `tid` neprejde nikdy", () => {
    ...
    expect(verifyOAuthProfile("microsoft", ms({ tid: undefined }), {}))
      .toEqual({ ok: false, dovod: "cudzi-tenant" })
    expect(verifyOAuthProfile("microsoft", ms({ tid: "" }), { allowedTenantIds: [] }))
      .toEqual({ ok: false, dovod: "cudzi-tenant" })
  })
  ...
  it("veľkosť písmen v `tid` nerozhoduje", () => {
    const r = verifyOAuthProfile("microsoft", ms({ tid: TID_SFZ.toUpperCase() }), {
  ...
  it("adresa sa vezme z `preferred_username`, keď `email` chýba", () => {
    ...
    // nastavené kontá.
    const r = verifyOAuthProfile("microsoft", ms({ email: undefined, preferred_username: "Jan.Letko@futbalsfz.sk" }), {})
    expect(r).toMatchObject({ ok: true, email: "jan.letko@futbalsfz.sk" })
  })

  it("adresa sa vezme z `upn`, keď chýbajú obe", () => {
    const r = verifyOAuthProfile("microsoft", ms({ email: undefined, preferred_username: undefined, upn: "a@b.sk" }), {})
    expect(r).toMatchObject({ ok: true, email: "a@b.sk" })
  })

  it("profil bez adresy neprejde", () => {
    const r = verifyOAuthProfile("microsoft", ms({ email: undefined }), {})
  ...
  it("hodnota bez zavináča nie je adresa", () => {
    ...
    const r = verifyOAuthProfile("microsoft", ms({ email: undefined, preferred_username: "jletko" }), {})
    expect(r).toEqual({ ok: false, dovod: "ziadna-adresa" })
  })

  it("prednosť má `oid` pred `sub` — je nemenné v rámci tenanta", () => {
    expect(verifyOAuthProfile("microsoft", ms(), {})).toMatchObject({ externalId: "abc-oid" })
    expect(verifyOAuthProfile("microsoft", ms({ oid: undefined }), {})).toMatchObject({ externalId: "abc-sub" })
  })
})

describe("Google", () => {
  it("overené konto z povolenej domény prejde", () => {
    const r = verifyOAuthProfile("google", google(), { hostedDomain: "futbalsfz.sk" })
  ...
  it("neoverená adresa neprejde", () => {
    ...
    expect(verifyOAuthProfile("google", google({ email_verified: false }), {}))
      .toEqual({ ok: false, dovod: "neovereny-email" })
    expect(verifyOAuthProfile("google", google({ email_verified: undefined }), {}))
      .toEqual({ ok: false, dovod: "neovereny-email" })
    expect(verifyOAuthProfile("google", google({ email_verified: "true" }), {}))
      .toEqual({ ok: false, dovod: "neovereny-email" })
  })

  it("konto z cudzej domény neprejde", () => {
    // `hd` v požiadavke je pre Google len nápoveda; vynucuje sa až tu.
    const r = verifyOAuthProfile("google", google({ hd: "inyzvaz.sk" }), { hostedDomain: "futbalsfz.sk" })
    expect(r).toEqual({ ok: false, dovod: "cudzia-domena" })
  })

  it("osobné konto (bez `hd`) neprejde, keď je doména vyžadovaná", () => {
    const r = verifyOAuthProfile("google", google({ hd: undefined }), { hostedDomain: "futbalsfz.sk" })
    expect(r).toEqual({ ok: false, dovod: "cudzia-domena" })
  })

  it("bez vyžadovanej domény prejde aj osobné konto", () => {
    const r = verifyOAuthProfile("google", google({ hd: undefined }), {})
    expect(r.ok).toBe(true)
  })
})
...
describe("co konto nedokazuje", () => {
  it("overenie nehovorí nič o tom, či človek patrí do organizácie", () => {
    ...
    const r = verifyOAuthProfile("microsoft", ms({ email: "ktokolvek@cudzia.sk" }), {})
