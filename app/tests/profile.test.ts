...
function profile(check: any = {}): TenantProfile {
  const p = defaultProfile("TEST")
  return {
    ...p, ...check,
    providers: {
      embedding:  { ...p.providers.embedding,  ...(check.providers?.embedding  ?? {}) },
      rerank:     { ...p.providers.rerank,     ...(check.providers?.rerank     ?? {}) },
      generation: { ...p.providers.generation, ...(check.providers?.generation ?? {}) },
    },
  }
}

// ── validacia ────────────────────────────────────────────────────────────
t("platny predvoleny profil prejde", throws(() => validateProfile(profile())) === null,
  String(throws(() => validateProfile(profile()))))

let e = throws(() => validateProfile(profile({ providers: { generation: { kind: "openai", url: undefined } } })))
t("openai bez url spadne", !!e && e.includes("url"), String(e))

e = throws(() => validateProfile(profile({ providers: { generation: { kind: "anthropic", url: "http://x" } } })))
t("anthropic s url spadne", !!e && e.includes("url"), String(e))

e = throws(() => validateProfile(profile({ providers: { generation: { kind: "openai", url: "http://v/v1", citations: true } } })))
t("citations pri openai spadnu", !!e && e.includes("citations"), String(e))

e = throws(() => validateProfile(profile({ dataResidency: "air-gap" })))
t("air-gap s atlas-auto spadne", !!e && e.includes("air-gap"), String(e))

e = throws(() => validateProfile(profile({
...
e = throws(() => validateProfile(profile({ providers: { embedding: { dim: 0 } } })))
t("nulova dimenzia spadne", !!e && e.includes("dim"), String(e))

// vectorPath — najcastejsia ticha chyba pri Automated Embedding
e = throws(() => validateProfile(profile({ providers: { embedding: { vectorPath: "embedding" } } })))
...
const pr2 = getProviders(profile({
...
e = throws(() => getProviders(profile({ providers: { embedding: { kind: "infinity" } } })))
t("factory: embedding infinity bez url spadne", !!e && e.includes("url"), String(e))

e = throws(() => getProviders(profile({ providers: { rerank: { kind: "tei" } } })))
t("factory: rerank tei bez url spadne", !!e && e.includes("url"), String(e))

const prOnprem = getProviders(profile({
  providers: {
...
t("factory: on-prem rerank sa robi v aplikacii, nie v DB",
  prOnprem.rerank.isPipelineStage === false && getProviders(profile()).rerank.isPipelineStage === true)

delete process.env.ANTHROPIC_API_KEY
e = throws(() => getProviders(profile()))
