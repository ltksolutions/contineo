...
- [x] **PR 14 — Správa platformy a Príručka (`ADMIN.md`)** ✅ implementované 2026-09-22 na vetve `design/pr14-admin` (stojí na PR 13, 4 commity) — **posledný PR handoffu**. **úloha 1.1** pilulky na `/admin` cez `.tag--draft` namiesto inline farby (vypnutá organizácia, dokumenty bez znenia). **úloha 1.2** čísla v poradí dokumenty, trasy, osoby, potvrdenia — Contineo je systém na dokumenty; zadanie píše „dokumenty, **znenia**, osoby, potvrdenia", znenia sa však na `/admin` nepočítajú (druhé číslo sú trasy) a nechal som ich na mieste, otázka v PR. Mriežka `.admin-data` nedotknutá. **úloha 1.4 (ADR-010)** `suggestCompanyCode()` + `withoutCollision()` v `lib/slug.ts` vedľa `slugifyKey()` (čisté funkcie, testy +2): iniciály bez diakritiky („Stredoslovenská vodárenská spoločnosť" → `SVS`), jednoslovný názov prvé tri písmená, spojky von, obsadený návrh dostane číslo; klientsky `CompanyCodeField` dopĺňa kód z názvu, **kým sa kódu nedotkne človek**, kolízia proti zoznamu načítanému pri otvorení (bez nového API), posledná brána je zápis. **Bez JavaScriptu** sú to dve obyčajné povinné polia a formulár sa odošle rovnako. Nápoveda: kód je súčasťou identifikátora každého dokumentu. Navyše: po chybe sa názov a kód vracajú do formulára (predtým sa strácali). **úloha 1.3** `.empty` „Žiadne organizácie". **úloha 1.5** organizácia bez domén dostala v karte varovanie `.ask-error` s vetou, čo to znamená — `.notice` je v Contineu **modálne okno**, na stav v karte sa nehodí; kľúč `noDomain` odišiel. **Príručka (2.1–2.3):** 2.2 je už splnená (`.answer` má `max-width: 70ch` z `ASK.md` úlohy 4 — 68ch zo zadania by znamenalo rozísť sa s ňou), 2.3 hovorí nerobiť nič; **2.1 nerobená** — zadanie popisuje prepínač kapitol ako `<Link className="card">`, ale `/guide` je jeden súvislý text z `content/guide.ts` a žiadny prepínač nemá; rozdeliť ho na kapitoly je stavba, nie diff. Otázka v PR.
      Overenie: tsc ✓, eslint 0 errors (42 = baseline), vitest 1398 ✓ (+2), build ✓; render svetlá 1440 + tmavá 390.
