...
- [ ] **Presun dokumentu do priečinka nezapisuje audit** — nájdené pri pláne D93 (2026-09-23). `assignDocument()` v `lib/folders.ts` nastaví len `updatedAt`/`updatedBy` a `writeAudit` nevolá, ani po jednom, ani hromadne; ostatné operácie s priečinkami ho volajú. Zápisy „audit zostáva" pri 4e boli nepravdivé a sú opravené. D93 PR 1.
- [ ] **Export CSV ignoruje filter oddelenia** — `library/csv/route.ts` neposiela `ownerDepartment`, obrazovka áno, takže export vráti viac, než človek vidí. Príčina sú dve ručné kópie mapovania `filters → LibraryFilter`; zjednotiť do jednej funkcie. D93 PR 0.
