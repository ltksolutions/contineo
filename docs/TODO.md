...
- [ ] **58 zlúčených vetiev sa povaľuje na `origin`** — po zlúčení handoffu (2026-09-22) je práca všetkých v `main`, `git branch -r --merged origin/main` ich vráti 58. Nemažú sa: **mazanie vetvy je výslovný súhlas Jána** a zmazaná vetva sa z GitHubu vracia ťažko. Keď sa raz budú upratovať, ide o dve dávky — lokálne (`git branch -d`, odmietne nezlúčené) a vzdialené (`git push origin --delete`). Vetvy zreťazeného handoffu `design/pr0-zaklad` … `design/pr14-admin` a `fix/po-handoffe` sú medzi nimi; sú to zároveň stopa, ako PR vznikali, takže ich hodnota nie je nulová.
