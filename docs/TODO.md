...
- [ ] **Slovenčine chýba piaty zdroj identity na webe** — nájdené **prvým behom testu zhody kľúčov** (2026-09-22). `tech.identity.providers` má v `sk` štyri položky, v `cs` a `en` päť: slovenčine chýba **„vlastná databáza"** a štvrtá položka je formulovaná inak („CRM / zdroj členstiev — pripravujeme" oproti „CRM / API zdroje identity"). Hlavný jazyk teda ukazuje **menšiu ponuku než cudzie mutácie**. Text sa nedopísal z hlavy — je to tvrdenie o produkte, nie preklep — a čaká na znenie od Jána. V `tests/dictionaries.test.mjs` je to menovitá výnimka so zdôvodnením a dátumom plus test, ktorý kontroluje, že výnimka ešte platí; po doplnení textu tie riadky zmiznú.
