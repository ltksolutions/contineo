...
- [ ] **Potvrdenie dokumentu bez JavaScriptu** — `AcknowledgeButton` posiela `fetch` na `/api/acknowledgements`, takže bez skriptu tlačidlo mlčí. Dnes je tam aspoň hláška, ale je to **právne záväzný úkon** (dôkazný záznam D24) a mal by ísť aj serverovým formulárom. Návrh: server action, ktorá zapíše to isté čo API (rovnaká `acknowledge()`, rovnaká formulka z `buildStatement`), tlačidlo zostáva klientske kvôli stavu odosielania. Treba doriešiť, čo s `ReadingTimer` — bez skriptu čas nemeria a záznam by mal prázdne `readingMs`
