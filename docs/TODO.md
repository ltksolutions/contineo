...
- [x] ~~**Tridsať nadbytočných `text-decoration: none` v `globals.css`**~~ ✅ 2026-09-23, PR #83 — odišlo 24 pravidiel a tri inline štýly; **päť zostalo** s dôvodom v komentári: `.button`, `.footer-link`, `.request-title` stoja aj v `<p>` (trieda prebije `p a`), `.bulk-clear` a `.directory-contact a` pod 640 px rušia vlastné podčiarknutie zo širšej obrazovky. `.bulk-clear` sa našiel až porovnaním vypočítaných štýlov v Chrome (390/1440 px, staré vs. nové CSS), nie čítaním. Pôvodný zápis: — od PR „Knižnica proti MASTER.md" (22. 9.) potláča podčiarknutie odkazov jedno pravidlo v koreni (`a { text-decoration: none }`) a vracia sa výslovne v bežnom texte. Tridsať pravidiel, ktoré si ho potláčali samy, tým stratilo dôvod — rovnako ako niekoľko inline `textDecoration: "none"` v kartách (`admin/page.tsx`, `people/page.tsx`, `hr/page.tsx`). Nič nekazia, len hovoria o probléme, ktorý už neexistuje. **Je to samostatné upratovanie, nie prílepok:** v jednom veľkom diffe by sa stratila vecná zmena, a pri každom pravidle treba overiť, že ho naozaj drží len koreň a nie niečo špecifickejšie. Zoznam sa vyrobí `grep -n 'text-decoration: none'` — k 2026-09-23 ich je **30**. Patrí sem aj `.notice-confirm`, ktoré si pravidlo lepí samo a od koreňového pravidla je zbytočné.
...
- [x] **58 zlúčených vetiev sa povaľuje na `origin`** ✅ vybavené 2026-09-22 so súhlasom Jána. Do mazania ich medzitým pribudlo na **64** (vetvy zo značky, ESLintu a Next 16). `git branch -r --no-merged origin/main` vrátil **nulu**, takže žiadna z nich nedržala prácu mimo `main`. Zmazalo sa oboje: vzdialené (`git push origin --delete` po dávkach) aj lokálne (`git branch -d`, nie `-D` — git tak sám odmietne nezlúčenú vetvu a poistka nestojí nič). **Zostal jediný `main`, lokálne aj na `origin`.** Stopa po tom, ako PR vznikali, žije ďalej v merge commitoch a v `git log`; tým sa nič nestratilo.
...
- [x] **PR 11 — HR: súhrn dopadu pred pridelením a pred pripomienkami (`HR.md`, úlohy 3–4)** ✅ implementované 2026-09-22 na vetve `design/pr11-hr-dopad` (stojí na PR 10, 2 commity) — **úloha 3** formulár `/hr/assign` beží bez skriptu, takže výber je na serveri až po odoslaní: pribudol krok **„Skontrolovať dopad"** (druhé tlačidlo, serverová akcia `previewAssignAction`, nič nezapíše — vráti výber v adrese vrátane termínu, ako návrat s chybou) a nad „Prideliť" `.assign-impact`: „Povinnosť vznikne N ľuďom" + rozpis po publikách + D50. Číslo cez `audienceImpact()` → `impactFrom()` → **`matchesAudience()`**, jediné miesto s pravidlom príslušnosti; osoby sa načítajú raz (`audiencePool()` zdieľa aj `audienceMembers()`); kto je v dvoch publikách, počíta sa raz. Bez publika tá istá chyba ako pri pridelení. Testy +3. Návrat s chybou teraz nesie aj termín (`dueMode/dueDate/dueDays`) — predtým sa strácal. **úloha 4** `/hr/reminders` ten istý blok s iným slovesom: „Odíde N e-mailov" + „Ľudia, ktorí už potvrdili, nedostanú nič (D61). Odoslaný e-mail sa odvolať nedá." (kľúč `reminders.preview` odišiel); hlásenie po odoslaní so skutočným počtom už bolo (`Odoslané: N.` na `/hr`) — nemenené (rozhodnutie Jána 2026-09-22).
      Odchýlka: súhrn v `/hr/assign` sa neukazuje živo pri klikaní (bez JavaScriptu sa nedá), ale až po kroku „Skontrolovať dopad" — rozhodnutie Jána 2026-09-22. Overenie: tsc ✓, eslint 0 errors (42 = baseline), vitest 1393 ✓, build ✓; render svetlá 1440 + tmavá 390.
- [x] **PR 12 — HR: dôkaz bez rozbaľovania a prázdne stavy (`HR.md`, úlohy 5–6)** ✅ implementované 2026-09-22 na vetve `design/pr12-hr-evidencia` (stojí na PR 11, 2 commity) — **úloha 5** `/hr/evidence`: dátum do `<summary>` vedľa pilulky (potvrdené → kedy; odvolané → kedy; inak kedy otvoril / „neotvorené"); v `.widget-body` `<dl class="detail-meta">` (ten istý komponent ako bočný panel detailu, žiadne nové triedy): IP · oddelenie v čase potvrdenia · znenie formulky · čas čítania · prvýkrát otvoril, pri odvolaní Odvolal · Dôvod. **Os s váhou riadkov ostáva pod dvojicami** (rozhodnutie Jána 2026-09-22 — nič sa nezmenšuje). Dáta: `evidenceRecords()` v `acknowledgements.ts` (posledné potvrdenie + platné odvolanie; pravidlo platnosti ako `isAcknowledged()` — odvolanie platí, kým ho neprebije potvrdenie s vyšším `cycle`; čistá časť `evidenceRecordsFrom()` s testami), `EvidenceRow.acknowledgement/revocation`; odvolanie ide do `dutyState()` ako `revokedAt`, takže pilulka povie „odvolané" (výkaz ho dovtedy videl len ako nepotvrdené). Filter evidencie ostáva na troch stavoch osi. **úloha 6** `.empty` na všetkých šiestich obrazovkách s textami zo zadania (sk/cs/en): `/hr` Žiadne pridelenia · `/hr/[id]` **Všetci potvrdili** (dobrá správa) · `/hr/overview` Zatiaľ niet čo zhrnúť · `/hr/assign` Niet čo prideliť · `/hr/reminders` Niet komu pripomínať (text podľa režimu: meškajúci vs. všetci nepotvrdení) · `/hr/evidence` dva stavy: „Filtru nič nevyhovuje" (keď záznamy sú, filter nič nenašiel) vs. „Žiadne záznamy". Vety k evidencii doplnené (zadanie dáva len nadpisy). Staré kľúče `empty`/`allAcknowledged`/`noEffectiveVersion`/`nothing` odišli.
      Overenie: tsc ✓, eslint 0 errors (42 = baseline), vitest 1396 ✓ (+3), build ✓; render svetlá 1440 + tmavá 390.
- [x] **PR 13 — Osoby (`OSOBY.md`, úlohy 1–5)** ✅ implementované 2026-09-22 na vetve `design/pr13-osoby` (stojí na PR 12, 5 commitov) — **úloha 1** `personTagClass()` v `lib/persons.ts` vedľa `PersonStatus`: aktívna `.tag--published`, pozvaná `.tag--review`, vyradená `.tag--archived`, neznámy stav neutrálny; volá ju `/people` aj `/people/[id]` (na karte pribudla pilulka vedľa mena). Role ostávajú neutrálne `.tag`. **Stav osoby a stav povinnosti sú dve škály a nezlučujú sa.** Stav „bez konta (len v adresári)" zo zadania **nerobený** — v modeli neexistuje (`PersonStatus` má tri hodnoty, adresár číta tú istú kolekciu), otázka v PR. **úloha 2** `.person-card.is-excluded { opacity: .75 }` — vyradená osoba je stlmená, ale z filtra neodchádza (D24). **úloha 3** `/people/[id]` do jedného stĺpca podľa `DETAIL.md`: opätovné pozvanie a vyradenie/vrátenie odišli do `<details class="detail-tools">` „Prístup a členstvo" (potvrdzovacie slovo pri vyradení ostáva), dôkazy dole; zoznam pridelených noriem kreslí `dutyTagClass()` z PR 10 (vrátane odvolania cez `revokedAt`) — `.evidence-state--*` z `globals.css` odišli, už ich nikto nekreslil. **úloha 4** `.empty`: `/people` rozlišuje prázdnu organizáciu od filtra bez zhody (+ „Zrušiť filter" cez `.empty-action`; počet sa kreslí len keď je čo počítať, kľúč `nothingFound` odišiel), `/people/invite` „Všetci sú pozvaní", karta osoby „Žiadne pridelené normy" (len personalistovi — komu sa zoznam nekreslí, nemá chýbať ani prázdny tvar). **úloha 5** `/people/import` hovorí **pred** nahraním, čo sa stane s existujúcou osobou — blok `.assign-impact` (nie `.notice`, tá je v Contineu modálne okno): spáruje sa podľa e-mailu a doplní sa, stav ani jazyk sa nemenia, nikoho nevyradí a nič nezmaže; druhá veta priznáva, že trasy, skupiny a roly sa nastavia podľa súboru a chýbajúci stĺpec ich vyprázdni. Text je opísané správanie `upsertPersons()` — kód nemenený, riziko do PR ako otázka.
      Overenie: tsc ✓, eslint 0 errors (42 = baseline), vitest 1396 ✓, build ✓; render svetlá 1440 + tmavá 390.
- [x] **PR 14 — Správa platformy a Príručka (`ADMIN.md`)** ✅ implementované 2026-09-22 na vetve `design/pr14-admin` (stojí na PR 13, 4 commity) — **posledný PR handoffu**. **úloha 1.1** pilulky na `/admin` cez `.tag--draft` namiesto inline farby (vypnutá organizácia, dokumenty bez znenia). **úloha 1.2** čísla v poradí **dokumenty, znenia, osoby, potvrdenia** podľa zadania: počet znení nahradil počet trás (znenia sú to, čo organizácia má; trasy sú vnútorná mechanika — rozhodnutie Jána 2026-09-22) a počítajú sa z dokumentov, ktoré `tenantOverviews()` už načítalo, takže **dotaz nepribudol, ale ubudol** (trasy sa už nepočítajú). Mriežka `.admin-data` nedotknutá. **úloha 1.4 (ADR-010)** `suggestCompanyCode()` + `withoutCollision()` v `lib/slug.ts` vedľa `slugifyKey()` (čisté funkcie, testy +2): iniciály bez diakritiky („Stredoslovenská vodárenská spoločnosť" → `SVS`), jednoslovný názov prvé tri písmená, spojky von, obsadený návrh dostane číslo; klientsky `CompanyCodeField` dopĺňa kód z názvu, **kým sa kódu nedotkne človek**, kolízia proti zoznamu načítanému pri otvorení (bez nového API), posledná brána je zápis. **Bez JavaScriptu** sú to dve obyčajné povinné polia a formulár sa odošle rovnako. Nápoveda: kód je súčasťou identifikátora každého dokumentu. Pri kolízii **server povie, ktorý kód je voľný** (tá istá `withoutCollision()`, akú používa formulár) — bez skriptu bolo „kód je obsadený" slepou uličkou; rozhodnutie Jána 2026-09-22. Navyše: po chybe sa názov a kód vracajú do formulára (predtým sa strácali). **úloha 1.3** `.empty` „Žiadne organizácie". **úloha 1.5** organizácia bez domén dostala v karte varovanie `.ask-error` s vetou, čo to znamená — `.notice` je v Contineu **modálne okno**, na stav v karte sa nehodí; kľúč `noDomain` odišiel. **Príručka (2.1–2.3):** 2.2 ostáva na `70ch` z `ASK.md` (jedna šírka riadka v systéme je viac než presné číslo — rozhodnutie Jána 2026-09-22), 2.3 hovorí nerobiť nič, **2.1 sa nerobí** — príručka je súvislý text, ktorý sa číta zhora nadol; keď narastie, príde obsah s odkazmi na `##` nadpisy, nie prepínač kapitol (zapísané medzi otvorenými vecami). **Navyše:** detail organizácie dostal blok „Čísla organizácie" — tie isté dlaždice ako prehľad (`Fact` vytiahnutý do `components/Fact.tsx`) **plus trasy**, ktoré z prehľadu odišli: prehľad slúži na porovnanie organizácií, detail na úplnosť (rozhodnutie Jána 2026-09-22). `tenantOverviews(code)` počíta len tú jednu organizáciu, `trackCount()` volá len detail — v prehľade dotaz na trasy nepribudol.
      Overenie: tsc ✓, eslint 0 errors (42 = baseline), vitest 1406 ✓, build ✓; render svetlá 1440 + tmavá 390.
- [x] **Oprava importu osôb — chýbajúci stĺpec už nemaže dáta** ✅ 2026-09-22 na vetve `design/pr14-admin` (nad PR 14; nájdené pri PR 13, rozhodnuté opraviť samostatne) — `upsertPersons()` zapisovalo `tracks`, `groups` a `roles` **vždy**, takže súbor bez týchto stĺpcov ich existujúcim ľuďom vyprázdnil; `roles` navyše CSV nerozpoznáva vôbec, takže **každý import zmazal roly každému, koho sa dotkol**. Teraz: zapisuje sa len to, čo v riadku naozaj je (`!== undefined`), `groupHistory` sa hýbe **spolu so skupinami** (keď riadok o skupinách mlčí, členstvo sa nemení), nová osoba dostane prázdne zoznamy cez `$setOnInsert` (schéma ich má povinné, existujúcej sa tadiaľto nič nedotkne). Rozdiel „prázdny stĺpec" (= vyprázdniť) vs. „chýbajúci stĺpec" (= nechať) vie len čítanie CSV, kde vidno hlavičky — `hasField()` v `personsImport.ts`; dovtedy prázdna bunka padala do `undefined` a vyprázdniť sa nedalo vôbec. Texty na `/people/import` upravené podľa nového správania (sk/cs/en). Testy: nový `personsUpsert.test.ts` (5 prípadov nad mockom kolekcie) + `personsImport.test.ts` (+2), spolu 1405 ✓. **CSV roly naďalej nerozpoznáva** — čítať ich by bola nová schopnosť s bezpečnostným dosahom (kto by importom mohol prideliť rolu `platform-admin`), samostatné rozhodnutie.
- [x] **PR 0–14 — celý handoff** ✅ 2026-09-22, zreťazené vetvy `design/pr0-zaklad` … `design/pr14-admin` (PR #46–#61). Tri otázky z pôvodného zápisu zodpovedané: „Uložiť pohľad" sa nerobí (adresa je uložený pohľad), termín v `acknowledgementDuties()` vyriešený pri PR 2, správanie importu pri existujúcej osobe napísané na obrazovku **a opravené** (mazalo roly).
- [x] **Oprava tabuľky knižnice proti MASTER.md** ✅ 2026-09-22, vetvy `fix/button-bez-podciarknutia` (PR #63) a `fix/kniznica-tabulka`. **Žiadny z piatich hlásených bodov nebol regresia z handoffu** — overené cez `git log -S`, všetky sú staršie než PR #46–#62 a handoff ich len zviditeľnil. **Pravidlo `.button`** (vlastný PR): `.button` nemal `text-decoration: none` nikdy, jediné globálne pravidlo je `a { color: inherit }` od prvého commitu (`a558860`, 27. 7.), takže 36 pravidiel s `text-decoration` v `globals.css` je rozsypaná záplata; pravidlo ide na `.button`, nie na `a` — podčiarknutie odkazu vo vete je správne. **Stĺpec Verzia**: label je v bunke od `5b18cfc` (7. 9.), `.doc-col-version` má `nowrap`, `max-width: 88px` a orezanie elipsou s plným znením v `title`; **„len číslo" sa spraviť nedá** — `Version.label` je voľný text („ľudské označenie: 1.2, novela 2026") a štruktúrované číslo v schéme nie je. Počet znení odišiel pod názov k `internalNumber`. **Šírka**: zúženie stĺpca nestačilo (chýbalo 122 px), knižnica dostala vlastný strop `--shell-maxw-wide: 1440px` cez `<AppShell wide>` — je jediná obrazovka s bočným panelom **aj** deväťstĺpcovou tabuľkou; strop, nie plná šírka okna (na 27" by riadok presiahol 2000 px). Pri 1440 px má stĺpec 1138 px, tabuľka potrebuje 1060, rezerva ~78 px na preklady. **„+ Podmienka"** presunutá do riadku hľadania, chróm karty je na `[open]`; builder a lišta vznikli s odstupom (`66e14d9` 8. 9. vs `7dd0e2c` 21. 9.) a nikto ich nespojil. **Značka v poli hľadania sa nestratila** — `ContineoMark` je v knižnici aj v hlavičke podľa zadania; problém je, že pri 16 px vyzerá jej silueta ako lupa, rieši sa prekreslením značky samostatne.
      Odchýlky: hlavičky dátumov premenované na „Platné od" / „Platné do" podľa MASTER (sk aj cs); anglická terminológia sa nemenila — „Effective from / to" je pojem z domény. Pod približne 1360 px tabuľka pretečie ďalej a je to zámer (`overflow-x: auto` bráni stlačeniu stĺpcov). `MASTER.md` dostal varovanie namiesto vety „zodpovedá dnešnému kódu" — tá veta bola horšia než nesprávny stĺpec, lebo odrádza od kontroly.
- [x] ~~**Prejsť obrazovky po pravidle `.button`**~~ ✅ 2026-09-22, vetva `fix/kniznica-proti-masteru` (PR #72). Prešlo sa **22 odkazov na jedenástich obrazovkách** (zadanie odhadovalo 24). **Bez zmeny 3:** názvy v kartách `/admin`, `/people`, `/hr` — mali inline `textDecoration: "none"` už predtým, pravidlo ich len spravilo nadbytočnými. **Zlepšenie 1:** názov trasy v `/library/tracks` bol jediný zo štyroch rovnakých kariet, ktorý podčiarknutie mal. **Zhoršili by sa 3 a doplnili sa do pravidla:** `.field-hint` a `.ask-error` sú bežné vety s odkazom uprostred, len nie sú v `<p>`, takže ich pravidlo zo zadania minulo (prázdne stavy na `/people/[id]`, chyba vo vyhľadávaní); `.directory-contact` je **vedomá odchýlka** — e-mail a telefón v adresári majú vyzerať ako odkazy, lebo adresár existuje práve na to, aby sa niekomu dalo ozvať. Zoznam je v tele PR #72. **`.notice-confirm` sa neodstránilo** — presunulo sa do bodu o tridsiatich nadbytočných pravidlách vyššie, kam patrí vecne (je to to isté upratovanie, nie doplnok prehliadky).
- [x] ~~**`ContineoMark` pri 16 px vyzerá ako lupa**~~ ✅ 2026-09-22, vetva `fix/znacka-pri-malej-velkosti`. Príčina **nebola hrúbka ťahu** — zmerané vedľa seba proti skutočnej lupe: aj s ťahom dopočítaným na konštantných ~2,2 px (pravidlo, ktoré pre ikony zavádza `Icon.tsx`) zostávala silueta lupy, len tučnejšia. Rozhoduje **kam chvostík mieri**: kružnica s ručkou pod 45° je lupa, nech je nakreslená akokoľvek hrubo. Chvostík preto visí zvisle dole a je širší než dlhší, ako sa kreslí bublina reči; kruh dostal hrubší ťah a väčšie bodky, aby oči pri 16 px prehovorili.
      Odchýlka od zadania: **jedna kresba pre všetky veľkosti**, nie variant pre 16 px. Dve siluety toho istého loga stávajú vedľa seba na jednej obrazovke — hlavička nesie značku pri 26 px a hneď vedľa ňu pole hľadania pri 16 px — a to je horšie než ktorákoľvek z tých dvoch kresieb samostatne. `strokeWidth` zostáva **pevný**, nie dopočítaný ako v `Icon.tsx`: ikona má byť opticky rovnaká v každej veľkosti, značka má s veľkosťou **mohutnieť** — dopočítaný ťah robil pri 40 px chudokrvný krúžok.
      Navyše: `SignIn.tsx` mal **vlastnú kópiu kresby** a pri prekreslení by v nej zostala stará značka. Záhlavie `ContineoMark.tsx` pritom presne toto predpovedá („kresba v SVG je presne ten druh veci, ktorý sa pri kopírovaní rozíde — jedna sa opraví, druhá zostane stará a nikto si to nevšimne"). Prihlásenie berie značku zo spoločného komponentu.
- [x] ~~**Značka na marketingovom webe zostáva stará**~~ ✅ 2026-09-22, vetva `fix/znacka-web-a-og`. **Bolo to sedem miest, nie dve.** Okrem `components/Logo.js` a `lib/og.js` ešte `app/icon.svg` (vlastná kresba vo svojej 32 mriežke) a štyri rastrové súbory z júna, ktoré **negeneruje žiadny skript**: `public/icon-192.png`, `public/icon-512.png`, `app/apple-icon.png`, `app/favicon.ico`. A tri kópie tej istej OG grafiky 1200×630: `public/og.png`, `app/opengraph-image.png`, `app/twitter-image.png` — na poslednú som prišiel až po tom, čo som si myslel, že som hotový.
      **Vecný nález na OG grafike:** nadpis hovorí „Opýtajte sa. Nehľadajte." a vedľa neho bola ako vodoznak nakreslená **lupa** (naša vlastná značka zväčšená na 470 px, ale čítala sa ako lupa). Obrázok, ktorý reprezentuje web pri každom zdieľaní, teda popieral vetu vedľa seba. Po prekreslení chvostíka to sedí.
      Rastre sú prekreslené z novej `app/icon.svg`, ktorá je odvodená priamo zo 48 mriežky cez `transform`, nie nakreslená znova — aby sa nemohla rozísť. `favicon.ico` nesie 16, 32 a 48 px ako predtým. OG grafika je zložená nanovo v rovnakej kompozícii, s Inter z `@fontsource/inter`, teda s tým istým fontom, aký používa generátor v `lib/og.js`. Dynamické OG obrázky pre `[lang]` routy generátor robí za behu a vodoznak nemali vôbec.
- [x] ~~**Zdieľaný odkaz `https://contineo.app` nemal náhľad**~~ ✅ 2026-09-22, vetva `chore/web-eslint-vitest`. **Značky za to nemohli** — overené na živom webe: koreň vracal kompletnú sadu `og:` aj `twitter:` značiek vrátane `og:image` a `/opengraph-image.png` vracal 200. Vinník bol **stavový kód**: `app/page.js` robil `redirect("/sk")`, teda **307**, a scraper, ktorý vidí 3xx, telo odpovede spravidla vôbec nečíta. Nové `middleware.js` koreň **prepisuje** na `/sk` (rewrite, nie redirect), takže adresa zostane a klient dostane **200** s celou stránkou. Duplicitný obsah nevzniká: `canonical` je `/sk`. `matcher` je výhradne `/`, aby middleware nebežal na obrázkoch a statických súboroch. Zvážené a zamietnuté: `permanentRedirect()` (308) je sémanticky správnejší než 307, ale náhľad by aj tak pripísal cieľovej adrese — a chceli sme ho pre tú, ktorú ľudia zdieľajú.
- [x] ~~**`web/` nemá nastavený ESLint ani testy**~~ ✅ 2026-09-22. ESLint podľa Next 14 (`.eslintrc.json` + `next lint`), lebo `eslint-config-next@14` plochý tvar nemá a `next lint` v Next 16 už neexistuje — rozdiel oproti `app/` je zapísaný vo `web/ESLINT.md` a pri povýšení na 16 sa zlúči. Lint 0/0: jediný nález (`<img>` v `Tech.js`) je vypnutý menovite na tom riadku aj s dôvodom — SVG diagram, ktorý `next/image` neoptimalizuje. Vitest úmyselne úzky, 15 testov: zhoda kľúčov `sk`/`cs`/`en`, prázdne texty, pád `getDictionary()` na slovenčinu, `skrat()` a `textyStranky()`. `textyStranky()` a `skrat()` sa presťahovali do nového `lib/ogTexty.js` — `og.js` obsahuje JSX a `next/og`, takže sa dá spustiť len v Nexte, hoci tie dve funkcie dostanú reťazec a vrátia reťazec.
- [x] ~~**Slovenčine chýba piaty zdroj identity na webe**~~ ✅ 2026-09-22 — **bolo to naopak.** Test zhody kľúčov našiel rozdiel, ktorý vyzeral ako chýbajúci preklad; po overení v kóde sa ukázalo, že `app/src/lib/auth.ts` má presne troch poskytovateľov (e-mailový odkaz, Microsoft Entra ID, Google), **žiadny `Credentials` provider** a záhlavie toho súboru hovorí doslova „prihlasuje sa odkazom v e-maile, bez hesla". Piata položka „Vlastní databáze / Own database — účty mimo SSO (credentials)" teda popisovala **funkciu, ktorá neexistuje**; slovenčine nič nechýbalo, cudzie mutácie sľubovali navyše. Stránka má pritom vlastnú konvenciu — čo ešte nie je, nesie „pripravujeme" — takže neoznačená položka tvrdí, že funkcia je hotová. Z `cs` aj `en` odišla.
      Štvrtá položka bola opačný prípad: slovenčina mala „CRM / zdroj členstiev — pripravujeme", čeština a angličtina výhradu nemali, hoci text o princípoch **o pár riadkov nižšie** vo všetkých troch jazykoch hovorí, že sa synchronizácia pripravuje. `externalRef.sportnetId` sa pri zakladání osoby vždy nastaví na `null` a synchronizačný kód nikde nie je. Výhrada doplnená do `cs` aj `en`; sportnet.online zostáva len v slovenčine.
      Výnimka v teste tým stratila dôvod a odišla aj s kontrolou, že ešte platí. **Dokumentácia je indícia, kód je pravda** — a tentokát to platilo aj pre marketingový web.
- [x] ~~**Výpadok produkcie: `/library/new` a `/admin/new` padali na funkcii v slovníku**~~ ✅ 2026-09-22, vetva `fix/funkcia-do-klienta` (PR #71). Hláška „A server error occurred", v runtime logoch Vercelu dôvod: **funkcia sa nedá serializovať cez hranicu servera a klienta**. Kľúče `keyTaken` a `codeTaken` boli napísané ako `(id: string) => string` a odovzdávali sa ako prop klientskym komponentom `KeyPreview.tsx` a `CompanyCodeField.tsx`. Prerobené na šablónu s `{id}`, ktorú si komponent dosadí sám. **`tsc` to nevie chytiť a nikdy nebude** — typ funkcie je platný typ, chybné je až prekročenie hranice RSC. Preto pribudol `tests/clientLabels.test.ts`: prejde každý `"use client"` súbor pod `src/components` a `src/app` a hľadá prop, ktorého typ sa končí na `=> string`. Overený tak, že sa chyba zámerne vrátila a test spadol.
- [x] ~~**Šesť nezhôd knižnice proti `MASTER.md` a `KNIZNICA.html` (rám 1)**~~ ✅ 2026-09-22, vetva `fix/kniznica-proti-masteru` (PR #72). **Päť zo šiestich nebola regresia** — overené `git log -S`, sú staršie než PR #46–#62. Čiastočná regresia bola jedna: stavová pilulka je nová (`49c7beb`, 21. 9.), ale napojila sa na existujúce **množné** facetové reťazce. Opravené: podčiarknutie koreňovo (viď bod vyššie); pilulka dostala reťazce v jednotnom čísle (Platný · Návrh · Na schválenie · Expirovaný, sk/cs/en) a **chýbajúcu vetvu pre expirovaný** — dovtedy prepadla do predvolenej a expirovaný dokument hlásil „koncept"; odvodenie je v `displayStatus()` vedľa `statusTagClass()` s `asOf` ako parametrom, nie `new Date()` vnútri; riadok pod názvom je `počet znení · interné číslo · priečinok`; v stĺpci Zmenené zostal len dátum; úvodný odsek z `/library/new` odišiel z knižnice; panel filtrov dostal jednotné nadpisy skupín a priečinky hore. **Značka sa nerobila** (MultiSelect, mimo rozsahu) a facet jazyka dostal vlastný nadpis, nie `t.status`.
- [x] ~~**Expirovaný nie je hodnota facetu, je to odvodený príznak**~~ ✅ 2026-09-23, vetvy `feat/builder-platne-do` (PR #73) a `feat/expired-cez-builder` (PR #74). Rozhodnutie Jána podľa `MASTER.md`, vykonané **v dvoch krokoch, nie naraz**: overil som, či query builder vie vyjadriť „Platné do · pred · dnes", a **nevedel** — pole `effectiveTo` v ňom nebolo a hodnota sa čítala cez `new Date(c.value)`, takže slovo „dnes" by skončilo ako `Invalid Date`. Napísal som to a facetu som sa nedotkol, kým to builder nevedel. **PR A** pridal pole `effectiveTo` s operátormi `pred`/`po` a token `today` (anglicky, ukladá sa ako token, nie ako dnešný dátum — odkaz poslaný dnes má o týždeň stále znamenať „dnes"). **Pasca:** koniec platnosti je na **znení**, nie na dokumente, takže naivné `effectiveTo < dnes` by vrátilo aj dokument s jedným starým a jedným platným znením; preto `$not: { $elemMatch: … }`. **PR B** facet odstránil a starý odkaz `?status=expired` prekladá na tú podmienku. Poistkou je test rovnosti z PR A: `expiredCondition()` **zostala samostatným kódom** a nevolá nový builder — keby volala, test by netvrdil nič.
- [x] **Nasadenie `KNIZNICA.html` rám po ráme** ✅ PR #75, zlúčený a nasadený 2026-09-23. Prejdených je všetkých osem rámov, sedem commitov. Postup, ktorý sa osvedčil a ktorý sa oplatí zopakovať pri ďalších obrazovkách: **vykresliť rám, vykresliť našu značku pod naším `globals.css` a porovnať vypočítané hodnoty prvok po prvku.** Čítaním CSS by sa nenašlo nič z toho, čo sa našlo — `span:last-child { margin-left: auto }` zarovnávalo „Nezaradené" vpravo, lebo ten riadok nemá počet; `auto-fill minmax(250px, 1fr)` dávalo na tablete 834 px tri stĺpce tam, kde rám má dva po 395 px; akcie v hlavičke končili na 973 px zo 1408, lebo chýbala medzera `flex: 1`. **Panel filtrov nebol karta** a prišlo sa na to až na druhý raz: v prvom kole sa porovnávalo pravidlo `.library-folders` proti pravidlu `.library-folders` a sedelo — vzhľad karty ale v ráme nesie **druhá trieda v tej istej značke** (`class="card library-folders"`). **Poučenie: porovnávať celý `class`, nie jeho hlavnú časť.** Zámerne neprevzaté z rámu je vypísané v tele PR #75.
- [ ] **`.page-head` nesedí s rámom o dve hodnoty** — medzera 12 px a spodný odstup 6 px proti rámovým 10 a 14 (`KNIZNICA.html`, rám 1). Triedu zdieľa `/hr/evidence`, takže zmena siaha mimo knižnicu a do PR #75 nepatrila. Rozhodnúť, či sa rám dotiahne na obe obrazovky, alebo či knižnica dostane vlastnú hodnotu.
- [x] ~~**Výpadok: knižnica s akýmkoľvek filtrom vracala chybu servera**~~ ✅ 2026-09-23, vetva `fix/tdz-v-kniznici` (PR #76). Po zlúčení PR #75. `ReferenceError: Cannot access 'aZ' before initialization` v `Array.map`, digest `1208623390`, routy `/library` a `/library.rsc`. **Príčina:** `activeNames` stálo nad `facetLabel` a siahalo naň — pole sa vyhodnocuje v mieste zápisu, takže `.map` bežal ihneď a `facetLabel` bol ešte v dočasnej mŕtvej zóne. Bez filtra vráti `activeChips(filters)` prázdne pole, callback sa nezavolá a k `facetLabel` sa nesiahne — preto to bolo vidieť len so zapnutým filtrom. Oprava je presun deklarácie, nič iné. **Dve poučenia o overovaní, nie o kóde:** (1) po nasadení som meral prázdnu knižnicu a ani raz nezafiltroval — overovalo sa to, čo sa menilo, namiesto toho, čo ľudia s obrazovkou robia; (2) prvý pokus reprodukovať pozeral len stavový kód a osem adries vrátilo 200 — **Next servíruje chybovú stránku s kódom 200**, hľadať treba v tele odpovede. Obe zapísané v `NEXT.md` medzi pravidlami overovania.
- [x] ~~**Zapnúť `no-use-before-define`**~~ ✅ 2026-09-23, vetva `chore/lint-use-before-define` (PR #77). Zo štyroch bŕzd nezastavila výpadok ani jedna: `tsc` hlási `TS2448` len pri **priamom** odkaze v tom istom mieste, a náš bol vnútri callbacku pre `.map`, teda pre kompilátor odložené vykonanie; testy tú stránku nevykresľujú; `next build` prešiel, je to chyba za behu. `@typescript-eslint/no-use-before-define` je preto **error**, nie warning — warning by dnešok nezastavil. Overené tak, že sa chyba vrátila späť do kódu: `tsc` naďalej nula, eslint dve chyby na riadku 232. Nastavenie `functions: false` (deklarácie funkcií sa vyťahujú nahor), `typedefs`/`enums: false` (typy za behu neexistujú). Pravidlo našlo ešte tri miesta a **ani jedno nebola chyba** — `fieldStyle` v `Rating.tsx` a `currentTenant` v `session.ts` by za behu prešli; presunuté, lebo inak sa pravidlo nedá zapnúť ako chyba. Čisté presuny deklarácií, žiadny riadok kódu sa nemenil.
- [ ] **`web/public/og.png` je mŕtvy súbor** — v kóde sa naň nikde neodkazuje a je bajtovo zhodný s `app/opengraph-image.png` (zistené 2026-09-22). Prekreslil som ho spolu s ostatnými, aby nezostal starý, ale dá sa zmazať. Mazanie súboru je výslovný súhlas, takže to čaká na rozhodnutie.
...
- [x] **4e. Hromadné akcie** — hromadný presun do priečinka (cyklus nad `assignDocument`) a odovzdanie výberu na `/hr/assign`, ktoré prideľovanie už vie. **Prideľovanie sa nepísalo druhýkrát.** Výber platí pre viditeľnú stranu; dávka môže skončiť čiastočne a vypíše, čo neprešlo
...
- [x] ~~**Dlhý výber napína adresu**~~ ✅ 2026-09-22 — `MAX_PICKED = 200` v `libraryFilters.ts` (rozhodnutie Jána 2026-09-22: rozumný strop). Odznačiť sa dá vždy, „označiť stranu" pridá len to, čo sa zmestí, a pás akcií strop povie nahlas — bez neho by sa adresa orezala a výber zmizol bez vysvetlenia.
- [ ] **„Všetko, čo vyhovuje filtru" namiesto zoznamu ID** — strop výberu rieši rezervu, nie princíp: adresa rastie lineárne s počtom označených. Kto označuje 148 dokumentov, v skutočnosti chce „všetko z tohto filtra" — to by bol jeden príznak (10 znakov) a server by zoznam dopočítal. Mení to sémantiku hromadnej akcie (medzi zobrazením a vykonaním sa obsah môže zmeniť), takže to chce vlastný plán a rozhodnutie, čo sa stane s dokumentom, ktorý medzitým z filtra vypadol. **Plán: `docs/D93_plan_vyber_podla_filtra.md`** (2026-09-23) — odporúčanie príznak v adrese + náhľad, ktorý zoznam zhmotní a pošle v tele POST; deväť otázok čaká na Jána.
- [x] ~~**Presun dokumentu do priečinka nezapisuje audit**~~ ✅ 2026-09-23 (PR #86, `51c1f26`) — `assignDocument()` zapíše `document` / `moved` s cestou názvov priečinkov („Normy / GDPR"); pôvodný priečinok z `findOneAndUpdate` s tvarom pred zápisom. Naživo neoverené (zápis do ostrej DB je nevratný), len testy `tests/assignDocument.test.ts`. Pôvodný zápis: — nájdené pri pláne D93 (2026-09-23). `assignDocument()` v `lib/folders.ts` nastaví len `updatedAt`/`updatedBy` a `writeAudit` nevolá, ani po jednom, ani hromadne; ostatné operácie s priečinkami ho volajú. Zápisy „audit zostáva" pri 4e boli nepravdivé a sú opravené. D93 PR 1.
- [x] ~~**Export CSV ignoruje filter oddelenia**~~ ✅ 2026-09-23 (PR #86, `30878ca`) — `listFilterOf()` v `libraryFilters.ts` je jediné mapovanie pre obrazovku aj export; overené na `sfz.localhost` (obrazovka = CSV pri piatich filtroch, produkcia pred opravou 0 vs. 10). Pôvodný zápis: — `library/csv/route.ts` neposiela `ownerDepartment`, obrazovka áno, takže export vráti viac, než človek vidí. Príčina sú dve ručné kópie mapovania `filters → LibraryFilter`; zjednotiť do jednej funkcie. D93 PR 0.
- [ ] **„z toho N mimo tohto zoznamu" počíta proti strane** — `pickedOutsideCount(filters.picked, pageIds)` v `library/page.tsx`; na strane 2 sa dokumenty označené na strane 1 hlásia ako „mimo", hoci filtru vyhovujú. Porovnávať s celým zoznamom. D93 PR 4.
- [x] ~~**Prázdny stav knižnice nerozlišuje „nič tu nie je" od „filtru nič nevyhovuje"**~~ ✅ 2026-09-23 (`19db418`, `accb500`) — prázdno z filtra vymenuje filtre, ktoré zoznam vyprázdnili; prázdna knižnica je iná stránka bez panela, hľadania a exportu. Pôvodný zápis: — nájdené 2026-09-22 pri testovaní na produkcii. Podmienka v `library/page.tsx` je `search ? t.nothingFound : t.empty`, teda pozerá **len na text hľadania**, nie na aktívne filtre. Keď filter vráti nulu, obrazovka napíše „Zatiaľ tu nie je nič. Začni nahratím prvého dokumentu" — aj keď je v knižnici desať dokumentov a len im nevyhovuje filter. Riadok je z 18. 9. (`893fa94`), takže to nie je regresia z handoffu; viditeľné je to ale až teraz, lebo **filter „expirované" vracia nulu najčastejšie**. Oprava je malá (pozrieť sa aj na `filters`, nielen na `search`), ale je to zmena textu na obrazovke, teda vlastné rozhodnutie: tretia veta by mala povedať „filtru nič nevyhovuje" a ponúknuť zrušenie filtra, nie nahrávanie dokumentu.
- [ ] **Stupne zhody môžu v praxi vyjsť všetky rovnaké** — pri skúške na produkcii (2026-09-22, otázka o lehote na námietku) dostalo všetkých päť zdrojov **„vysoká zhoda"**. Dáva to zmysel: `matchLevel()` počíta podiel voči najlepšiemu zdroju tej istej odpovede a po reranku sú prvé zdroje tesne pri sebe, takže podiel takmer vždy prekročí 0,8. Otázka je, či trojstupňová škála potom niečo hovorí, alebo je to päťkrát tá istá nálepka. Treba pozrieť rozloženie podielov na väčšej vzorke odpovedí a podľa toho buď posunúť prahy, alebo stupeň ukazovať len vtedy, keď sa zdroje naozaj líšia. **Neladiť prahy od stola** — bez dát by to bolo hádanie.
...
- [ ] **História zmien na osobe** — audit záznamy o úpravách osoby **sú** (`writeAudit()` pri `savePersonAction`, `togglePersonStatusAction`), na karte `/people/[id]` sa ale nevypisujú. `OSOBY.md` to vedie ako 🔴 rozhodnutie; Ján 2026-09-22 rozhodol **nerobiť teraz**. Pri doplnení treba povedať, čo sa ukazuje (kto, kedy, ktoré pole) a či to vidí len personalista — je to výpis o človeku, nie o dokumente.
- [ ] **Obsah príručky, keď narastie** — `/guide` je jeden súvislý text (`content/guide.ts`), ktorý sa číta zhora nadol; `ADMIN.md` úloha 2.1 žiadala prepínač kapitol, ale ten v kóde nikdy nebol a rozdelenie by z čítania spravilo orientovanie sa. Rozhodnutie Jána 2026-09-22: **nerobiť**. Keď bude príručka dlhá natoľko, že to prekáža, správna odpoveď je **obsah s odkazmi na `##` nadpisy** (kotvy v tom istom texte), nie prepínač kapitol.
...
- [ ] **O15, O16 — právny základ a retencia** `acknowledgements` (DPO, právnik) — rozširuje D10. **Prvé kolo zodpovedané 2026-09-23** (15/21, `docs/O15_O16_otazky_pre_DPO.md`); právny základ implementovaný ako **D91 + D92**. **Druhé kolo odoslané 2026-09-23, zodpovedané 2026-09-24** (A8–A11, B1a, B4a–B7, B10a/b, B11 — zapísané v `docs/O15_O16_otazky_pre_DPO.md`); **otvorené:** počet rokov stropu (B10a, príklad 5), termín balančného testu, C1–C4 (C4 rieši Miška/Lukáš), kontrola číselníka `Pravne_zaklady_navrh_ciselnika.docx` sa nevrátila. Teraz: TTL pre `acknowledgements`, `assignments`, `document_opens`, `approval_rounds`; postup pri výmaze a námietke (čl. 21); úprava vety v `GDPR_DATA_PROTECTION.md` §6.
  ...
  - [ ] **Zaviesť lehoty z druhého kola:** 3 roky od skončenia pomeru/vzťahu (`acknowledgements`, `assignments`, `document_opens`), poistka od vyradenia, `approval_rounds` s posledným potvrdením znenia, TTL 24 mesiacov na `audit`. Strop B10a až po potvrdení čísla.
  - [ ] **Evidencia námietky (A8)** — záznam a obrazovka; doklad sa do rozhodnutia nemaže.
  - [ ] **Štvrťročný výkaz pre DPO** — predpisy a ich právne základy (A10).
