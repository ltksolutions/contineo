...
- [ ] **O15, O16 — právny základ a retencia** `acknowledgements` (DPO, právnik) — rozširuje D10. **Prvé kolo zodpovedané 2026-09-23** (15/21, `docs/O15_O16_otazky_pre_DPO.md`); právny základ implementovaný ako **D91 + D92**. **Druhé kolo odoslané 2026-09-23** (`O15_O16_otazky_pre_DPO_kolo2.docx`) spolu s kontrolou číselníka (`Pravne_zaklady_navrh_ciselnika.docx`). Po odpovedi: TTL pre `acknowledgements`, `assignments`, `document_opens`, `approval_rounds`; postup pri výmaze a námietke (čl. 21); úprava vety v `GDPR_DATA_PROTECTION.md` §6.
