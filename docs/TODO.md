...
- [ ] **Povinnosti z trasy sa nedá dať vedieť e-mailom** (nájdené 2026-09-08 pri skúške na skúšobnej smernici). „Dať vedieť" (`/hr/[id]/notify`) je postavené **nad prideleniami**: obrazovka ho ponúka len vtedy, keď má pridelenie aspoň jedného nepotvrdeného. Keď povinnosť vznikne **krokom v trase**, pridelenie neexistuje, takže tlačidlo nie je nikde — a to je práve cesta, ktorou chodí onboarding.
      Vidieť to na skutočných dátach: vo výkaze má „Skúšobná trasa" **0 z 1, chýba 1**, dokument 1.1 tiež „chýba 1" — a ponúknuť sa nedá nič.
      Jediná existujúca cesta sú **Pripomienky**, tie ale berú len meškanie nad prahom. Prah sa dá zmeniť adresou (`/hr/reminders?days=1`), lenže **na nulu nie**: `Math.max(1, Number(q.days) || DEFAULT_DAYS)` má dve zábrany naraz — jednotku ako dolnú hranicu a `||`, cez ktoré `days=0` prepadne na 14. Pre povinnosť, ktorá vznikla dnes, teda e-mail neexistuje; zajtra už áno.
      Návrh (rozhodnúť pred realizáciou): zovšeobecniť „dať vedieť" nad `duties()` z `hrReport.ts` — ten už spája pridelenia aj trasy a je to ten istý zdroj, z ktorého počítajú Pripomienky. „Dať vedieť hneď" a „pripomenúť meškajúcim" sú potom tá istá obrazovka s prahom 0 a 14 dní. Lacnejšia náplasť je povoliť prah 0 (`Math.max(0, …)` a `q.days ?? DEFAULT_DAYS`), ale nechá to nesúlad v pomenovaní: „pripomienka" pre niečo, čo človek ešte nemal ako stihnúť.
