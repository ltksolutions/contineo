...
- [ ] **O17 — pseudonymizácia v `evaluations`** (audit 2026-09-16). **Smer rozhodnutý 2026-09-16 (Ján): `personId`, nie e-mail.** Plán realizácie a tri otvorené otázky: `docs/O17_plan_personid_v_hodnoteniach.md`. `GDPR_DATA_PROTECTION.md` kap. 3 má zásadu „`userId`/`sessionId` pseudonymizovať", ale kolekcia ukladá e-mail pýtajúceho aj hodnotiteľa **doslovne**. Buď sa to pseudonymizuje, alebo sa zásada prepíše — **rozhodnutie, nie implementácia**.
- [ ] **O18 — potvrdiť zero-retention a región pre Voyage (embedding + rerank)** (audit 2026-09-16, upresnené po otázke Jána). Text opúšťa infraštruktúru na dvoch miestach — pri vektoroch a preradení (Voyage cez Atlas) a pri **generovaní** (Anthropic). Pri Anthropicu je zmluvný rámec známy; pri Voyage má riadok v tabuľke sub-procesorov stále poznámku „overiť zero-retention + región" a tá otázka je otvorená od júna. **Z pohľadu GDPR ide o otázku človeka**, nie o znenie predpisu — predpis nie je osobný údaj a jeho dôvernosť rieši zmluva, nie retencia.
