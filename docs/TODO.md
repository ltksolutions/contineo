...
- [ ] **Logo v upozorňovacích e-mailoch je relatívna adresa** (zistené 2026-09-17 pri audite D90) — cron, schvaľovanie a pozvánky posielajú `/api/brand/…` bez domény; prihlasovací e-mail to rieši v `auth.ts`. Overiť v skutočnej schránke a opraviť na jednom mieste (napr. absolútna adresa v `brandingView()` pre e-mail).
- [ ] **Región funkcií na Verceli je `iad1` (USA)**, Atlas vo Frankfurte — rozhodnúť presun na `fra1` (`regions` vo `vercel.json`); súvisí s ADR-002 a O12 (Static IPs sa nastavujú pre región).
