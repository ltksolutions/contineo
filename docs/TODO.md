...
- [ ] **Značka na marketingovom webe zostáva stará** — `web/components/Logo.js` a `web/lib/og.js` nesú **ďalšie dve kópie** starej kresby (nájdené 2026-09-22 pri prekreslení značky). `web/` je samostatné nasadenie a zmena loga aj OG obrázkov je zmena značky **navonok**, nie oprava rozhrania — preto som sa ich nedotkol a čaká to na rozhodnutie. Kým sa to nespraví, contineo.app a intranet nesú dve rôzne značky. Kresba sa medzi `app/` a `web/` zdieľať nedá (dve samostatné aplikácie), takže kópia tam zostane — otázka je len, či bude aktuálna. Mocky v `docs/design/*.html` sú história handoffu a nechávajú sa tak.
