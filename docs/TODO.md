...
- [ ] **Prejsť obrazovky po pravidle `.button`** — 24 odkazov-tlačidiel na 11 obrazovkách (`/admin`, `/library` vrátane stránkovania a panela filtrov, `/library/curation`, `/people`, `/hr`, `/hr/overview`, `/hr/evidence`, `/acknowledgements`, `/documents`, `/not-found`, `Notice`) odteraz nie je podčiarknutých. Nájsť, čo sa tým zmenilo k horšiemu, a doriešiť zvlášť. Zámerne nie v PR s pravidlom (rozhodnutie Jána 2026-09-22): najprv pravidlo, potom prehliadka. `.notice-confirm` si `text-decoration: none` lepí sám a je odteraz zbytočné — odstrániť pri tejto prehliadke, nie skôr.
