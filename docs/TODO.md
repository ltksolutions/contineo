...
- [x] ✅ 2026-08-28 **Cesta cez `persons` je overená.** Predchádzajúci zápis tvrdil opak a bol nesprávny: `POVOLENE_EMAILY` je v produkcii **prázdna** (`vercel env pull`), takže núdzová brzda nepúšťa nikoho a prihlásenie muselo prejsť cez `persons`. Že prešlo, dokazuje živá relácia — widget sa vykreslil, čo vyžaduje nájdenú osobu v `persons` (`onboardingContext` → `ready`). Náhradný test druhou osobou tým stratil dôvod.
- [x] ✅ 2026-08-28 **Opravené: evidencia prihlásenia sa nestíhala zapísať.** Práve to overovanie odhalilo, že jediná osoba v `persons` má platnú reláciu a pritom `lastLoginAt` prázdne a stav stále `invited`. Príčina: `void recordSignIn(...)` bez `await` — funkcia na Verceli skončí hneď po vrátení hodnoty a rozrobený zápis do Atlasu sa zahodí. Nezasvieti pri tom nič. Na `lastLoginAt` má stáť príznak „nové" (D39), takže by sa to prejavilo až v rozsahu B ako nefunkčná funkcia inde. Regresný test v `tests/signIn.test.ts`.
- [ ] **Zostáva overiť prvé prihlásenie odznova** — `lastLoginAt` a `firstLoginAt` sa zapíšu až pri najbližšom prihlásení; skontrolovať `npm run stav`, že sa stav prepol `invited → active`.
