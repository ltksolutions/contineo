...
Presné poradie zľava. **Toto je cieľ, nie popis kódu** — v jednom bode sa
s ním kód rozchádza (stĺpec 5) a je to úloha, nie kontrola.

⚠️ Skoršia verzia tohto odstavca tvrdila, že tabuľka „zodpovedá dnešnému
kódu, takže sa nemení". Nebola to pravda a bola to horšia chyba než preklep:
veta odrádzala od kontroly presne tam, kde kontrola bola potrebná.
**Pri každom stĺpci over kód, netvrď to podľa tejto tabuľky.**
...
| 5 | **Verzia** | vľavo | `tabular-nums`, `nowrap` — **len číslo**. Kód tu má „Platné znenie" s celým `effectiveLabel` (napr. „1.2 (skúšobné znenie na overenie schvaľovania)"), ktorý sa zalomí do piatich riadkov, nafúkne riadok na ~150 px a vytlačí Potvrdenia a Zmenené mimo obrazovku. Počet znení patrí pod názov k `internalNumber`. |
